...
import { ModelManager } from "../llm";
import { Userdetails } from "../schema";

describe("Agent Runtime Loop & ReAct Execution Scenarios", () => {
  let callLlmSpy: any;

  beforeEach(() => {
    callLlmSpy = vi.spyOn(ModelManager.prototype, "call_llm");
  });
  ...
  it("should successfully run Scenario 1: Data Science MLOps Update", async () => {
  ...
  it("should successfully run Scenario 2: AI Engineering Web/Express Update", async () => {
    const user: Userdetails = {
      user_id: "usr_9910",
      name: "Vicky B",
    ...
    const runtime = new AgentRuntime(
      ...
      3500,
      user,
      "Add Express Framework to month 4 of my AI engineering roadmap and save it.",
      []
    );
    ...
    const state = runtime.getState();
    expect(state.roadmap).toBeDefined();
    expect(state.roadmap?.id).toBe("rdmp_aie2027");

    const month4 = state.roadmap?.months.find((m) => m.month === 4);
    expect(month4).toBeDefined();
    expect(month4?.activities).toContain("Express Framework");
  });

  it("should successfully run Scenario 3: Cybersecurity Search & Update", async () => {
    const user: Userdetails = {
      user_id: "usr_7720",
      name: "Ash",
    ...
    const runtime = new AgentRuntime(
      ...
      3500,
      user,
      "Search for ethical hacking labs and add Kali Linux Labs to month 3 of my cybersecurity roadmap.",
      []
    );
    ...
    const state = runtime.getState();
    expect(state.roadmap).toBeDefined();
    expect(state.roadmap?.id).toBe("rdmp_cyber2028");

    const month3 = state.roadmap?.months.find((m) => m.month === 3);
    expect(month3).toBeDefined();
    expect(month3?.activities).toContain("Kali Linux Labs");
  });
});
