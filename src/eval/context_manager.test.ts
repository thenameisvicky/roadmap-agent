...
describe("ContextManager Compaction & Eviction", () => {
  const contextManager = new ContextManager();

  it("should compact transfer learning session history items", () => {
    ...
    const hasCompactedDecision = context.trace.context_decisions.some(d => 
      d.reason.includes("transfer learning")
    );
    expect(hasCompactedDecision).toBe(true);

    const assistantMsg = context.messages.find(m => m.role === "assistant");
    expect(assistantMsg?.content).toBe("Previous discussion about transfer learning.");
  });

  it("should compact roadmap loaded in tool results", () => {
    const state: AgentState = {
      user_profile: undefined,
      roadmap: undefined,
      tool_results: [
    ...
    expect(decision?.reason).toContain("After get_roadmap, full roadmap JSON should not remain at full size");
    
    const userMsg = context.messages.find(m => m.role === "user");
    expect(userMsg?.content).toContain("Roadmap ID: rdmp_9f2a");
  });

  it("should evict oldest low-priority session history when budget is tight", () => {
    const state: AgentState = {
      user_profile: undefined,
      roadmap: undefined,
      tool_results: [],
    ...
    expect(context.trace.context_evicted.length).toBeGreaterThan(0);
    ...
    expect(isOldestEvicted).toBe(true);
  });
});
